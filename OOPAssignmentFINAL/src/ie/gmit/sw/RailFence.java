...
public class RailFence {
	...
	public String encrypt(String cypherText, int key){
		...
		//allows to take spaces while encrypting
		...
		char[][] matrix = new char[key][cypher.length()]; //The array is filled with chars with initial values of zero, i.e. '0'.
		
		//Fill the array
		int row = 0; //Used to keep track of rows
		boolean down = true; //Used to zigzag
		for (int i = 0; i < cypher.length(); i++){ //Loop over the plaintext
			matrix[row][i] = cypher.charAt(i); //Add the next character in the plaintext to the array
